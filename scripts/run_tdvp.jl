...
Delta_i = 3.0    # initial Ising anisotropy (ground state is prepared here)
Delta_f = 0.5    # final Ising anisotropy (time evolution under this)
...
chi_max_dmrg = 128    # maximum bond dimension for DMRG
n_sweeps     = 30     # total DMRG sweeps
cutoff_dmrg  = 1e-10  # SVD truncation cutoff for DMRG

chi_min_dmrg = max(chi_max_dmrg ÷ 8, 2)   # starting bond dimension for ramp
...
lanczos_max_iter   = 100   # maximum Lanczos restarts
...
cutoff_tdvp  = 1e-10   # SVD truncation cutoff during TDVP

# Krylov exponential solver parameters (for the local time evolution in TDVP)
krylov_dim  = 30       # Krylov subspace dimension (larger = more accurate per step)
krylov_tol  = 1e-12    # convergence tolerance for Krylov exponential
...
schedule = SweepSchedule(chi_max_dmrg, n_sweeps; chi_min=chi_min_dmrg, cutoff_final=cutoff_dmrg)
...
for sweep in 1:schedule.n_sweeps
    ...
    E_gs = dmrg_sweep(state, solver_dmrg, opts, dir)
    println("  $(lpad(sweep, 3))     $(lpad(schedule.maxdims[sweep], 4))   $(schedule.cutoffs[sweep])   $E_gs")
end
...
push!(norms, measure_norm(state.mps))
push!(m_stag_t, m_stag_init)
push!(sz_profiles, sz_init)

println("  step     t        energy              M_stag          norm")
println("  " * "─"^60)
println("  $(lpad(0, 4))   $(lpad("0.000", 8))    $(round(energies[1], digits=10))   $(round(m_stag_init, digits=8))   $(round(norms[1], digits=8))")

for step in 1:n_steps
    global state

    # One full TDVP time step = right sweep + left sweep
    ...
    if step % measure_every == 0 || step == n_steps
        ...
        println("  $(lpad(step, 4))   $(lpad(round(t, digits=3), 8))    $(round(E, digits=10))   $(round(m_stag, digits=8))   $(round(real(nrm), digits=8))")
    end
end
