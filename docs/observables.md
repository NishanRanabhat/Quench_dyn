...
## Finite-temperature measurements (purification)

Thermal expectation values `<O>_beta = Tr(rho O)` at inverse temperature `beta`
can now be measured, where `rho = exp(-beta H)/Z` is generated by imaginary-time
TDVP of an ancilla purification (see `docs/finite_T_purification_protocol.md`
and `scripts/run_finite_T.jl`).

The finite-T state is an **ordinary MPS** with local dimension `d^2 = 4`
(physical ⊗ ancilla merged into one leg). Because of that merge, **all the MPS
measurement functions above work unchanged** — no ancilla-aware measurement code
is needed. There are only two rules:

1. **Embed** the physical operator onto the merged leg with `embed_physical`.
2. **Normalize** by the trace `Tr rho = <psi|psi> = measure_norm(state.mps)`
   (thermal expectations are ratios).

### `embed_physical(op::Matrix; d=2) -> Matrix`

Embed a physical single-site operator into the purified local space as
`O ⊗ I_anc = kron(O, I_d)` (a `d^2 x d^2` matrix). The `⊗ I_anc` performs the
ancilla trace automatically when contracted.

```julia
Sz  = embed_physical(Matrix{Float64}(spin_ops(d)[:Z]); d=d)   # S^z ⊗ I_anc  (4x4)
```

### Thermal one- and two-point functions

For any physical single-site operators `O`, `P`, using the existing MPS routines:

```julia
# <O_i>_beta
val = measure_local_observable(state.mps, embed_physical(O; d=d), i) /
      measure_norm(state.mps)

# <O_i P_j>_beta
c = measure_correlation(state.mps, embed_physical(O; d=d), i,
                                   embed_physical(P; d=d), j) / measure_norm(state.mps)

# energy density <H>_beta / N  (the purified MPO already carries H ⊗ I)
E = measure_energy(state) / measure_norm(state.mps) / N
```

### `thermal_sz_from_purification(state::MPSState; d=2) -> (m, C)`

Convenience wrapper for the S^z channel: returns the thermal one-point profile
`m[j] = <S^z_j>_beta` and two-point matrix `C[i,j] = <S^z_i S^z_j>_beta`, already
embedded and normalized.

```julia
m, C = thermal_sz_from_purification(state; d=d)
```

Feed `C` (or the connected matrix `connected_matrix(C, m)`) to the coarsening
observables for the CDW / Néel channel:

```julia
rs, G = staggered_correlator(C)              # staggered (q=pi) real-space correlator
xi    = domain_length(rs, G)                 # staggered correlation length xi(T)
Sq    = structure_factor(C, [Float64(pi)])   # structure factor; q=pi peak = CDW
ms2   = staggered_magnetization_sq(C)         # <m_s^2> = S(pi)/N
```

### What can be measured

- **Energy density** `<H>_beta / N` — cooling diagnostic / temperature matching.
- **Longitudinal (CDW) channel** `<S^z_i S^z_j>` → `xi(T)`, `S(q)`, `<m_s^2>`.
- **Local magnetization** `<S^z_i>` (≡ 0 at zero field by symmetry).
- **Transverse (XY) channel** `<S^+_i S^-_j>` — embed `spin_ops(d)[:Sp]` /
  `[:Sm]`; physically distinct from the longitudinal channel in XXZ.
- **Connected correlators** `connected_matrix(C, m)` (= `C` when `m ≡ 0`).
- **Any physical operator** via `embed_physical` + normalize.
- **Thermodynamics** — internal energy `U(beta)`, specific heat (finite-difference
  `dU/dT`, or `beta^2 Var(H)`), entropy by integration. Note `energy_variance`
  assumes a unit-norm state; normalize by `Tr rho` for the purification.

See `docs/finite_T_purification_protocol.md` for the full method, conventions
(merged index, `beta = 2*tau`), and scaling notes.
